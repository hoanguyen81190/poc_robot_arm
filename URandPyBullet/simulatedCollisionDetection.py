...
class simulationCollisionDetection:
    def __init__(self):
        ...
        self.robot_id = p.loadURDF("urdf/ur16e_final.urdf", basePosition=[0, 0, 0.03], useFixedBase=True)

        self.num_joints = p.getNumJoints(self.robot_id) - 2 # Subtract 4 to exclude the fixed joints from the urdf from te beginning and end
    ...
    def simulate_path_to_target(self, start_joint_angles, start_position, start_orientation, target_position=[0,0,1.3], num_segments=10):
        ...
        intermediate_positions = np.asarray([start_position + (target_position - start_position) * (i / num_segments) for i in range(num_segments + 1)])
        ...
        intermediate_joint_angles = np.zeros((intermediate_positions.shape[0],self.num_joints))
        ...
        for i, pos in enumerate(intermediate_positions):
            ...
            print("Before norm Step: ",i," Joint angles: ", np.rad2deg(joint_angles))
            #joint_angles = np.asarray([clamp_angle(normalize_angle(angle)) for angle in joint_angles])
            #print("After norm Step: ",i," Joint angles: ", np.rad2deg(joint_angles))
